...
void mSDLGLES2Runloop(struct mSDLRenderer* renderer, void* user) {
	...
	while (mCoreThreadIsActive(context)) {
		...
		bool gotFrame = mCoreSyncWaitFrameStart(&context->impl->sync);
		if (gotFrame) {
			v->postFrame(v, renderer->outputBuffer);
		}
		mCoreSyncWaitFrameEnd(&context->impl->sync);
		{
			// Detect GBA-blocked freeze: log when the GBA stops producing frames.
			static int noFrameStreak = 0;
			if (!gotFrame) {
				noFrameStreak++;
				if (noFrameStreak == 30) { // ~1.5s of no GBA frames
					__android_log_print(ANDROID_LOG_WARN, "mGBA_Freeze",
					    "GBA not producing frames (WaitFrameStart timeout streak=%d)", noFrameStreak);
				}
			} else if (noFrameStreak >= 30) {
				__android_log_print(ANDROID_LOG_DEBUG, "mGBA_Freeze",
				    "GBA resumed producing frames after %d misses", noFrameStreak);
				noFrameStreak = 0;
			} else {
				noFrameStreak = 0;
			}
