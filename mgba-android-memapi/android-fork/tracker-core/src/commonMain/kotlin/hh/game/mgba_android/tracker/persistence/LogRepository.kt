 ...
 *
 * Mirrors the Lua `RandomizerLog.hasParsedThisLog` behavior: the parse is cached in-memory keyed
 * by ROM filename, so re-opening the log viewer is instant and only re-parses when the ROM (and
 * therefore the log) changes.
 */
object LogRepository {
    ...
    /**
     * Returns the parsed log for [romFileName], reading + parsing off the main thread on first
     * request and serving the cache thereafter.
     */
    suspend fun getLog(
        source: LogSource,
        romFileName: String,
        ...
        val lines = source.readLines(romFileName) ?: return@withContext Result.NotFound
