...
### Fixed
- **Max FR (gen4) tracking corrected — the variant is FireRed-based, not Emerald-based** — the gen4 address table (`max-fr-gen4.json`) had all of its battle-global *and* save-config addresses mistakenly copied from Emerald, while its party addresses were FireRed-based. As a result `gBattleMons`/`gBattlersCount` pointed *inside* the enemy-party memory (single battles misread as doubles, enemy unfindable; live stat stages showing −6 and Fire type flipping to Normal on battle entry), and the SaveBlock pointer/offsets were wrong (the Stats screen showed nothing or garbage). Corrected all runtime addresses to the FireRed-consistent values that match the working gen5 variant: 12 battle globals (battle mons, battler count, party indexes, type flags, outcome, weather, side statuses/timers, communication, hit marker, move-result flags, battle results) plus the save-config (`gSaveBlock1/2ptr`, `EncryptionKeyOffset` = 0xF20, `gameStatsOffset` = 0x120D, `gameFlagsOffset` = 0xEE0).
