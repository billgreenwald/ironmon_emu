...
- **Max FR (gen4) tracking corrected — the variant is FireRed-based, not Emerald-based** — the gen4 address table (`max-fr-gen4.json`) had all of its battle-global *and* save-config addresses mistakenly copied from Emerald, while its party addresses were FireRed-based. As a result `gBattleMons`/`gBattlersCount` pointed *inside* the enemy-party memory (single battles misread as doubles, enemy unfindable; live stat stages showing −6 and Fire type flipping to Normal on battle entry), and the SaveBlock pointer/offsets were wrong (the Stats screen showed nothing or garbage). Corrected the runtime addresses to the FireRed-consistent values that match the working gen5 variant: 12 battle globals (battle mons, battler count, party indexes, type flags, outcome, weather, side statuses/timers, communication, hit marker, move-result flags, battle results) plus the SaveBlock pointers (`gSaveBlock1/2ptr` = 0x3005008 / 0x300500C, `EncryptionKeyOffset` = 0xF20). Battle tracking and route reading now work on gen4.
- **Game stats hidden on Max FR (gen4)** — the SaveBlock1 game-stats offset for this variant could not be resolved (the stats block reads all-zero/garbage at every candidate offset), so the steps / battles / center-visits row is hidden on gen4 rather than showing wrong numbers. All other variants are unaffected.
...
- **Gallery image load diagnostics** — Glide load state (loading/success with size/failure with reason) is now logged under the `Gallery` tag and captured by Export Debug Logs.
...
### Fixed
- **MaxFR Gen5 / Gen4 species names** — Gen4 Pokémon on MaxFR Gen5 FR and MaxFR Gen4 ROMs were mapped to wrong names (e.g. Skuntank appearing as Hippowdon). The internal species ID base is now correctly set to 412, matching the NatDex extension species ID scheme used by these ROM hacks.
...
- **MaxFR Gen5 species/BST/ability/move data from Lua** — Gen5 FireRed and Gen4 variants now load species names, BSTs, ability descriptions, and move descriptions directly from the ROM hack's bundled Lua data files (`gen5.lua`, `gen4.lua`, `abilities.lua`, `moves.lua`). This fixes incorrect BST display (e.g. Cottonee showing Audino's BST) and ensures kaizo-buffed stats and custom abilities/moves show correct data. Ability IDs are offset-corrected (+1 shift) to match the ROM hack's internal numbering. Sprites for extended species now resolve to the correct Ironmon NatDex tracker sprite ID (national dex +25 offset), fixing wrong sprites such as Cottonee showing as Unfezant. Move category icons (physical/special/status) are now correctly decoded from ROM bit 6 of the move flags byte instead of bit 5, fixing most moves incorrectly showing the status (pink) icon.
